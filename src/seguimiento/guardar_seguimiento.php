...
try {
  ...
  if (count($incidencias) > 0) {
    $stmtInc = $conn->prepare('INSERT INTO seguimiento_incidencias (seguimiento_id, tipo, severidad, fecha) VALUES (?,?,?,?)');
    if (!$stmtInc) {
    ...
    foreach ($incidencias as $inc) {
      ...
      $stmtInc->bind_param('isss', $seguimientoId, $tipo, $sev, $fecha);
      if (!$stmtInc->execute()) {
